...
export const PostContent = ({ post, onDismiss }: IProps) => {
  ...
  const handleDragDismiss = (latest: ResolvedValues) => {
    if (+latest.y > 10) onDismiss?.(); // todo: debug this delta
  };
  return (
    !!post && (
      <motion.div
        ...
        <Card className="sm:px-5">
          <CardHeader>
            ...
            <motion.img
              ...
              className="!-mt-14 select-none rounded-xl shadow-2xl shadow-cyan-500/15 object-cover aspect-video w-full h-auto z-30"
              src={post.thumbnail}
              alt={post.title}
